...
- [x] Explicit Connection Type contracts and READY Runtime injection into Capability/Trigger Providers
...
- [x] Unified If condition / ForEach items expression fields with typed variable filtering, diagnostics, autosave, and undo/redo
- [x] Credential metadata/type Query and create/rotate/delete Actions with sanitized errors and version fencing
- [x] Responsive Credential management route with masked write-only forms, Connection usage protection, and live invalidation
...
- [x] Revision-owned Automation input declarations, defaults, and compile-time input-name diagnostics
...
- **Manual Run submission seam — pass:** the browser creates one immutable requestId per submission and retains the exact payload after an uncertain response. Scheduler acceptance transactionally stores the Run, `RunAccepted` journal event, payload hash, and request record; an identical retry returns the original runId across process restarts, while requestId reuse with different content returns `409 MANUAL_RUN_REQUEST_CONFLICT`. Revision fencing and input validation still happen before first acceptance, but cannot invalidate recovery of an already accepted request. Tests cover migration, same-process retry, payload mismatch, restart recovery after activation changes, Provider error projection, and the full suite.
...
- **Structured control authoring seam — pass:** If condition and ForEach items use the existing ValueExpr Field Shell and one typed Source command, preserving branch/body identity and concurrency. Array/object fields filter variable and Call choices by schema type. Full-document history survives in-flight saves, and Source diagnostics remain visible in the Inspector. Production-browser QA verifies reference save/reload, literal-array and Call editing, and 1440px/390px layouts without console errors.

- **Credential management seam — pass:** CredentialService owns type lifecycle, encrypted writes, version-fenced deletion, and transactional Connection-reference protection. Workbench projects only metadata and secret field shapes, omitting Schema defaults, descriptions, and options. A dedicated Page under Connections keeps masked secret input temporary, preserves the version captured when its form opens, aborts on disposal, and reconciles through coalesced invalidations. Production-browser QA covers create, rotate, concurrent-version conflict/reopen, Connection binding protection, delete, secret-free response bodies, and 1440px/390px layouts.
...
- Connection desired state, explicit Type/Adapter contracts, READY Runtime injection, generation-fenced create/update/delete/enable Actions, Adapter Schema configuration UI, generation-fenced Runtime recreation, and metadata-only Credential selection are operational. Credential metadata, creation/rotation/deletion Actions and UI are operational; automatic reconnect policy remains planned work. The shared outbound HTTP substrate supports scoped clients, timeout, cancellation, status errors, explicit proxy configuration, `NUMEN_HTTP_PROXY`, and HTTP(S)/SOCKS proxy protocols; `NO_PROXY` routing remains planned. The built-in bounded `http:request` Action supports text/JSON requests and responses but not multipart, binary Resource output, cookie jars, or per-step proxy selection. The Echo demo has no Connection or Credential and therefore does not validate a Credential-backed external Integration. Credential writes require an available type plugin and a configured runtime master key. Non-object secret contracts require a plugin-provided editor.
