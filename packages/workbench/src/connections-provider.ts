...
const connectionAdapterSchema = z.object({
  ...
  typeVersion: z.number().step(1).min(1).required(),
  typeTitle: z.string().required(),
...
function projectAdapter(ctx: Context, definition: ConnectionAdapterDefinition): WorkbenchConnectionAdapter {
  ...
  return {
    ...
    typeId: definition.type.id,
    typeVersion: definition.type.version,
    typeTitle: ctx.connections.getType(definition.type)?.title ?? definition.type.id,
