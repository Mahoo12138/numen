...
export const LogsView = defineSetupComponent<LogsViewProps>('LogsView', ['consoleClient', 'automationId', 'runId', 'compact'], props => {
  ...
  return () => <section class={['logs-view', props.compact ? 'logs-compact' : '']} aria-label={t('workbench.logs.title')}>
    <div class="logs-toolbar">
      ...
      <Button variant="secondary" class="secondary-button" onClick={() => { following.value = !following.value }} type="button">{t(following.value ? 'workbench.logs.pause' : 'workbench.logs.follow')}</Button>
      <Button variant="secondary" class="secondary-button" onClick={newest} type="button">{t('workbench.logs.latest')}</Button>
    </div>
    {!props.consoleClient ? <p>{t('workbench.logs.disconnected')}</p> : <>
      ...
      {feed.failed.value ? <p role="alert">{t('workbench.logs.failed')} <Button variant="secondary" class="secondary-button" type="button" onClick={feed.reload}>{t('workbench.tryAgain')}</Button></p> : null}
      ...
      <Button variant="secondary" class="secondary-button" disabled={!feed.snapshot.value?.next || feed.loading.value} onClick={() => { before.value = feed.snapshot.value?.next; following.value = false }} type="button">{t('workbench.logs.older')}</Button>
    </>}
  </section>
})
