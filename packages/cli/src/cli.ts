...
export async function runCli(argv = process.argv.slice(2), io = defaultIo): Promise<number> {
  ...
  if (command === 'config' && positionals[1] === 'validate') {
    ...
    createRuntimeEntries(loaded.config, new Set(['database', 'capabilities', 'automations', 'scheduler', 'triggers', 'server', 'health', 'readiness']))
  ...
  if (command === 'doctor') {
    ...
    const entries = createRuntimeEntries(
      loaded.config,
      new Set(['database', 'capabilities', 'automations', 'scheduler', 'triggers', 'server', 'health', 'readiness']),
      values.safe,
    )
    let writable = true
    try {
